                ...
                if (!_RadarCenterObject3D.Instanced)
                {
                    ...
                    if (_RadarCenterObject3D.UseTrackingLine)
                    {
                        ...
                        if (_RadarCenterObject3D.UseBaseTracker)
                        {
                            ...
                            baseTrackerRenderer.sprite = _RadarCenterObject3D.BaseTracker;
                    ...
                    if (_RadarCenterObject3D._CreateBlipAs == CreateBlipAs.AsSprite)
                    {
                        ...
                        blipSpriteRenderer.material = _RadarCenterObject3D.SpriteMaterial;
                        blipSpriteRenderer.color = _RadarCenterObject3D.colour;
                        blipSpriteRenderer.sprite = _RadarCenterObject3D.icon;
    ...
    Vector3.ClampMagnitude(RadarDesign.Pan / RadarDesign.SceneScale, RadarDesign.TrackingBounds);
            ...
            foreach (var item in Blips)
            {
                ...
                if (item.IsActive)
                {
                        ...
                        if (item._CreateBlipAs == CreateBlipAs.AsSprite)
                        {
                            ...
                            if (item.UseTrackingLine)
                            {
                                ...
                                if (item.UseBaseTracker)
                                {
                                    ...
                                    var baseTrackerRenderer = baseTracker.GetComponent<SpriteRenderer>();
                                    baseTracker.transform.parent = pivot.transform;
                                    baseTrackerRenderer.sprite = item.BaseTracker;
                            ...
                            blip.GetComponent<SpriteRenderer>().sharedMaterial = item.SpriteMaterial;
                            blip.GetComponent<SpriteRenderer>().color = item.colour;
                            blip.GetComponent<SpriteRenderer>().sprite = item.icon;
                    ...
                    for (var i = 0; i < trackedObjectCount; i++)
                    {
                        ...
                        var localYOffset = !item.KeepBlipsAboveRadarPlane ? transform.InverseTransformPoint(radarBlipPos - playerpos).y * transform.lossyScale.x : Mathf.Clamp(transform.InverseTransformPoint(radarBlipPos - playerpos).y * transform.lossyScale.x, 0, Mathf.Infinity);
                        // nV3 just mean new vector3
                        var nV3 = new Vector3(
                            transform.InverseTransformPoint(radarBlipPos - playerpos).x * transform.lossyScale.x,
                            localYOffset,
                            transform.InverseTransformPoint(radarBlipPos - playerpos).z * transform.lossyScale.x);
        ...
        public void Minimap()
        {
            ...
            if (!minimapModule.generated)
            {
                ...
                mask.transform.localScale = transform.localScale;
            ...
            if (minimapModule.mapType == MapType.Static)
                minimapModule.Map.transform.localPosition = -(new Vector3(
                                                                centerObjectsPosition
                                                                    .x - RadarDesign.Pan.x,
                                                                0,
                                                                centerObjectsPosition
                                                                    .z - RadarDesign.Pan.z) + rotationOffset)
        ...
        private void RotateSpecifics()
        {
            ...
            foreach (var rotationTarget in RadarDesign.RotationTargets)
            {
                ...
                switch (rotationTarget.ValueState)
                {
                    // s represents the word stte ,  positive 1 being the postive state and negative 1 being the negative state
                ...
                {
                    case RetargetX.none:
                        x = rotationTarget.AddedXRotation * s;
                        break;
                    case RetargetX.X_to_Y:
                        x = rotationTarget.Target.transform.eulerAngles.y / rotationTarget.RotationDamping * s;
                        break;
                    case RetargetX.X_to_Z:
                        x = rotationTarget.Target.transform.eulerAngles.z / rotationTarget.RotationDamping * s;
                        break;
                ...
                {
                    case RetargetY.none:
                        y = rotationTarget.AddedYRotation * s;
                        break;
                    case RetargetY.Y_to_X:
                        y = rotationTarget.Target.transform.eulerAngles.x / rotationTarget.RotationDamping * s;
                        break;
                    case RetargetY.Y_to_Z:
                        y = rotationTarget.Target.transform.eulerAngles.z / rotationTarget.RotationDamping * s;
                        break;
                ...
                {
                    case RetargetZ.none:
                        z = rotationTarget.AddedZRotation * s;
                        break;
                    case RetargetZ.Z_to_X:
                        z = rotationTarget.Target.transform.eulerAngles.x / rotationTarget.RotationDamping * s;
                        break;
                    case RetargetZ.Z_to_Y:
                        z = rotationTarget.Target.transform.eulerAngles.y / rotationTarget.RotationDamping * s;
                        break;
                    ...
                    case Rotations.Proportional:
                        ...
                        rotationTarget.TargetedObject.transform.eulerAngles = new Vector3(
                            rotationTarget.FreezeX
                                ? rotationTarget.AddedXRotation * s
                                : rotationTarget.retargetedXRotation == RetargetX.none
                                    ? (rotationTarget.Target.transform.eulerAngles.x /
                                       rotationTarget.RotationDamping + rotationTarget.AddedXRotation) * s
                                    : x,

                            rotationTarget.FreezeY
                                ? rotationTarget.AddedYRotation * s
                                : rotationTarget.retargetedYRotation == RetargetY.none
                                    ? (rotationTarget.Target.transform.eulerAngles.y /
                                      rotationTarget.RotationDamping + rotationTarget.AddedYRotation) * s
                                    : y,
                            rotationTarget.FreezeZ
                                ? rotationTarget.AddedZRotation * s
                                : rotationTarget.retargetedZRotation == RetargetZ.none
                                    ? (rotationTarget.Target.transform.eulerAngles.z /
                                      rotationTarget.RotationDamping + rotationTarget.AddedZRotation) * s
                                    : z
                        );
        ...
        /* public Vector3 MultiplyPoint(Vector3 v)
         {
             Vector3 result;
             result.x = 1 * v.x + 0 * v.y + 0 * v.z + 0;
             result.y = 0 * v.x + 1 * v.y + 0 * v.z + 0;
             result.z = 0 * v.x + 0 * v.y + 1 * v.z + 0;
             var num = 0 * v.x + 0 * v.y + 0 * v.z + 1;
             num = 1f / num;
             result.x *= num;
             result.y *= num;
             result.z *= num;
        ...
        /// </summary>
        [HideInInspector]
        public RadarDesign3D RadarDesign;
